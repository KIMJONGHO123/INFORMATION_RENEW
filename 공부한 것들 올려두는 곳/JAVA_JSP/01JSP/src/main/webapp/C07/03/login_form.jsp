...
<form action="${pageContext.request.contextPath}/C07/03/login.jsp" method="post">
	<div>
		...
		<input type="text" name="username" value="${cookie.ididid != null ? cookie.ididid.value:''}"/>
	</div>                                   <!-- value 안에 들어가면 "" 넣어야한다. -->
	...
	<div>
		<input type="checkbox" id="idSave" name="idSave" ${cookie.idchk !=null ? cookie.idchk.value: ''} />
		<label for="idSave">ID저장</label>
		
		<input type="checkbox" id="pwSave" name = "pwSave" ${cookie.pwchk !=null ? cookie.pwchk.value:''}/>
