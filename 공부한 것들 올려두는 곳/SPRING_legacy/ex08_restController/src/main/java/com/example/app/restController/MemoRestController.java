...
public class MemoRestController {
	...
	private MemoServiceImpl memoService;
		...
		public void add(@RequestBody MemoDto dto) throws SQLException {
			log.info("POST /memo/rest/memo/post.."+dto);
			memoService.registrationMemo(dto);
		}
		
		@GetMapping(value="/add/{id}/{text}")
		public void add_get( MemoDto dto) throws SQLException {
			log.info("GET /memo/rest/memo/post.."+dto);
			memoService.registrationMemo(dto);
		}
		
		//메모수정 
		
		// put 방식은 URL로도 전송가능하고 @RequestBody로도 전송 가능하다.
		// put 방식은 전체자원 교체
		// patch는 자원의 일부만 수정
		...
		@PutMapping("/put2")	//http://localhost:8090/ex08_restController/memo/put2
